    ...
    <nav className="bg-white fixed w-full z-10">
          ...
          <div className="hidden md:flex items-center space-x-8 ">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                className="text-gray-600 hover:text-gray-900 px-3 py-2 rounded-md  font-medium transition duration-300 text-xl"
              >
                {link.name}
              </Link>
            ))}
      ...
      <div className={`${isOpen ? "block" : "hidden"} md:hidden`}>
        <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
          ...
          <button className="w-full bg-blue-600 text-white px-6 py-2 rounded-md text-base font-medium hover:bg-blue-700 transition duration-300">
            login
          </button>
        </div>
      </div>
